...
def insert_sort(array):
    if len(array) == 0:
        return []
    result = [None]  # guardsman
    for i in range(len(array)):
        result.append(None)
        result[0] = array[i]  # update guardsman
        # we are going from the end of result list
        # but we exclude last None element
        k = len(result) - 1
        while k > 0:
            k -= 1
            if array[i] < result[k]:
...
def bucket_sort(array, k=10, next_sort=insert_sort):
    # zwykłem mnożenie tworzy k odwołąn do tej samej listy
